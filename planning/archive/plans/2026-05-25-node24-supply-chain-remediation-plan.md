...
- Apply the same policy to the temporary `pnpm-workspace.yaml` generated by `consumer-smoke:pack` so local tarball smoke tests exercise the same install-time guardrails.
- Add `scripts/dependency-policy.test.mjs` assertions for both root policy keys and consumer smoke policy propagation.
- Keep `minimumReleaseAgeExclude` absent by default. Any exception must be narrow, documented in the PR body, and covered by dependency policy tests.
