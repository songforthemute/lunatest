...
export function parseExternalConsumerProofOptions(args) {
  ...
  return {
    ...
    releasePackageSet: args.includes("--release-package-set"),
  };
}
...
function applyFixturePackageVersions(manifest, versions) {
  for (const [packageName, version] of Object.entries(versions)) {
    ...
    } else if (Object.hasOwn(manifest.devDependencies ?? {}, packageName)) {
      manifest.devDependencies[packageName] = version;
    } else {
      throw new Error(`Fixture does not declare public package ${packageName}`);
    }
  }
}

function readRegistryLatestVersions(names) {
  return Object.fromEntries(
    names.map((name) => {
      const output = run("npm", ["view", `${name}@latest`, "version", "--json"], repositoryRoot);
      const version = JSON.parse(output);
      if (typeof version !== "string") {
        throw new Error(`npm latest for ${name} did not resolve to one exact version`);
      }
      return [name, version];
    }),
  );
...
export function runExternalConsumerProof(lane, options = {}) {
  ...
  try {
    ...
    if (lane === "pack") {
      ...
      expectedVersions = options.releasePackageSet
        ? repositoryPackageVersions(publicPackages)
        : fixturePackageVersions(fixtureManifest, names);
      if (options.releasePackageSet) {
        applyFixturePackageVersions(fixtureManifest, expectedVersions);
        writeFileSync(
      ...
      registryLatestVersions = readRegistryLatestVersions(names);
      const installStarted = performance.now();
      run(
        "pnpm",
        ["install", options.releasePackageSet ? "--no-frozen-lockfile" : "--frozen-lockfile"],
        consumerDir,
        { stdio: "inherit" },
      );
      setupTiming.consumerInstallMs = performance.now() - installStarted;
    ...
    if (lane === "registry") {
      assertRegistryPackageSet({
        expectedVersions,
        latestVersions: registryLatestVersions,
        lockfile,
        manifest: readJson(join(consumerDir, "package.json")),
        packageNames: names,
      });
    ...
    const report = createExternalConsumerProofReport({
      ...
      packages: Object.entries(expectedVersions).map(([name, version]) => ({
        name,
        source: lane === "registry" ? "npm-registry" : "packed-tarball",
        version,
      })),
    ...
    const outputPath = resolve(
      repositoryRoot,
      options.outputPath ?? `artifacts/external-consumer-proof/${lane}.json`,
    );
