# @lunatest/e2e-tests

## 0.1.7

### Patch Changes

### Packages

- `@lunatest/core@0.2.1`: dependency range update
- `@lunatest/cli@0.1.6`: dependency range update
- `@lunatest/mcp@0.1.6`: dependency range update
- `@lunatest/playwright-plugin@0.2.2`: dependency range update
- `@lunatest/vitest-plugin@0.2.2`: dependency range update

## 0.1.6

### Patch Changes

### Packages

- `@lunatest/vitest-plugin@0.2.1`: dependency range update
- `@lunatest/playwright-plugin@0.2.1`: dependency range update

## 0.1.5

### Patch Changes

### Packages

- `@lunatest/core@0.2.0`: dependency range update
- `@lunatest/vitest-plugin@0.2.0`: dependency range update
- `@lunatest/playwright-plugin@0.2.0`: dependency range update
- `@lunatest/cli@0.1.5`: dependency range update
- `@lunatest/mcp@0.1.5`: dependency range update

## 0.1.4

### Patch Changes

### Packages

- `@lunatest/core@0.1.4`: dependency range update
- `@lunatest/cli@0.1.4`: dependency range update
- `@lunatest/mcp@0.1.4`: dependency range update

## 0.1.3

### Patch Changes

### Packages

- `@lunatest/core@0.1.3`: dependency range update
- `@lunatest/cli@0.1.3`: dependency range update
- `@lunatest/mcp@0.1.3`: dependency range update

## 0.1.2

### Patch Changes

### Packages

- `@lunatest/core@0.1.2`: dependency range update
- `@lunatest/runtime-intercept@0.1.1`: dependency range update
- `@lunatest/cli@0.1.2`: dependency range update
- `@lunatest/mcp@0.1.2`: dependency range update
- `@lunatest/playwright-plugin@0.1.2`: dependency range update

## 0.1.1

### Patch Changes

### Packages

- `@lunatest/cli@0.1.1`: dependency range update
- `@lunatest/mcp@0.1.1`: dependency range update
- `@lunatest/playwright-plugin@0.1.1`: dependency range update
