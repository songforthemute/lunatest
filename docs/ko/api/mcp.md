...
실행 파일은 서버를 만들기 전에 Lua scenario, coverage metadata/catalog, component/prompt 컨텍스트, project-local preset resource를 로드합니다. config에서 로드한 scenario ID는 `.lua`를 제거한 프로젝트 상대 경로이며, 예를 들면 `lunatest`, `scenarios/swap`입니다. `scenario.create`, `scenario.mutate`는 프로세스 메모리에서만 동작하고 `scenarioDir`에 쓰지 않습니다.
