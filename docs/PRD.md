...
│                                                                 │
│  ✦ 완전 결정론 — 동일 시나리오 = 항상 동일 결과 (no flaky)      │
...
## 10. 구현 페이즈 — Coding Agent Session Guide
...
```
┌──────────────────┬──────────────┬──────────────┬──────────────┐
│                  │ MSW / Mock   │ Anvil / Fork │  LunaTest    │
├──────────────────┼──────────────┼──────────────┼──────────────┤
│ 모킹 지점        │ HTTP 트랜스포트│ 없음 (실제)  │ 도메인 로직   │
│ 시나리오 언어     │ hex (ABI)    │ 체인 상태     │ Lua 테이블    │
│ 결정론           │ ✅           │ ❌ 블록 타이밍 │ ✅           │
│ 테스트 속도       │ ~5-20ms      │ ~1-10s       │ ~0.1-1ms     │
│ 프론트 버그 격리  │ △ 노이즈      │ △ 체인 에러   │ ✅ 순수 격리  │
│ AI 시나리오 생성  │ 어려움       │ 어려움        │ 용이          │
│ CI 비용          │ 낮음         │ 높음          │ 낮음          │
└──────────────────┴──────────────┴──────────────┴──────────────┘
```
